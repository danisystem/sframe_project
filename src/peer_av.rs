...
use image::ColorType; // usiamo solo l’encoder JPEG coi raw bytes
...
// Preferenza formato camera (MJPEG > YUYV > altro; vicino a W/H/FPS voluti)
fn pick_best_format(
...
fn main() -> Result<()> {
            ...
            loop {
                ...
                match sid {
                    SID_VIDEO if recv_video => {
                        ...
                        // Decodifica JPEG in RGBA8 (usa la versione di `image` presente a progetto)
    ...
    if send_video {
        ...
        thread::spawn(move || {
            ...
            loop {
                // Prendi frame RGB come RAW BYTES, evitando tipi di `image` concreti
                let rgb_raw: Vec<u8> = match cam.frame() {
                    Ok(f) => f.decode_image::<RgbFormat>().expect("rgb").into_raw(),
                    Err(e) => {
                ...
                // JPEG (nessun uso di RgbImage/ImageBuffer, passiamo i raw bytes)
                jpeg_buf.clear();
                let mut enc = image::codecs::jpeg::JpegEncoder::new_with_quality(&mut jpeg_buf, quality);
                if let Err(e) = enc.encode(&rgb_raw, use_w, use_h, ColorType::Rgb8) {
